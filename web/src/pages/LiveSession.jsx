...
export default function LiveSession({ appointmentId, setActivePage }) {
  ...
  const handleSendWhatsapp = async (phoneNum) => {
    ...
    if (whatsappTarget === 'note') {
      ...
      let msg = `*منصة مسبار للذكاء الاصطناعي الطبي*\n`;
  ...
  const handlePrintLetter = () => {
    ...
    win.document.write(`
      ...
      <body>
        ...
        <div class="footer-container" style="justify-content: ${isAr ? 'flex-start' : 'flex-end'};">
                    ...
                    <div className="w-full space-y-4">
                      {transcriptLines.length === 0 ? (
                        ...
                        transcriptLines.map(line => (
                          <div key={line.id} className="flex gap-3 bg-white p-4 rounded-xl border border-border-subtle shadow-xs items-start">
                            <div className="w-8 h-8 rounded-lg bg-[#006973]/10 text-[#006973] flex items-center justify-center font-bold text-xs shrink-0 border border-[#006973]/20">
                              <span className="material-symbols-outlined text-[16px]">record_voice_over</span>
                            </div>
                            <div className="flex-1 pt-0.5 text-sm text-on-surface leading-relaxed font-normal">
                              {line.text}
                            </div>
                          </div>
                        ))
                    ...
                    {isRecording ? (
                      ...
                      <>
                        {transcriptLines.length === 0 ? (
                          ...
                          transcriptLines.map(line => (
                            <div key={line.id} className="flex gap-3 animate-fade-in bg-white p-4 rounded-xl border border-border-subtle shadow-xs items-start">
                              <div className="w-8 h-8 rounded-lg bg-[#006973]/10 text-[#006973] flex items-center justify-center font-bold text-xs shrink-0 border border-[#006973]/20">
                                <span className="material-symbols-outlined text-[16px]">record_voice_over</span>
                              </div>
                              <div className="flex-1 pt-0.5 text-sm text-on-surface leading-relaxed font-normal">
                ...
                {whatsappShowPhoneInput ? (
                  <div>
                    <p className="text-xs text-secondary mb-4 leading-relaxed">
                      {isArabic 
                        ? `رقم الهاتف غير مسجل في النظام. يرجى إدخال رقم المستلم مع كود الدولة لإرسال التقرير (مثال: 966501234567):`
                        : `Phone number is not registered. Please enter recipient's phone number with country code (e.g. 966501234567):`}
                    </p>
                    ...
                    <div className="mb-5">
                      <input 
                        ...
                        placeholder={isArabic ? 'رقم الهاتف (مثال: 966501234567)' : 'Phone Number (e.g. 966501234567)'}

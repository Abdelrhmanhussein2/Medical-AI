...
export default function OrgDoctors() {
  ...
  // Filter roster by search & specialty filter
  const filteredDocs = deptDocs.filter(d => {
    const matchesSearch = d.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                          d.email?.toLowerCase().includes(searchQuery.toLowerCase());
    const docSpec = d.specialization || d.department || 'General Practice';
    const matchesSpecialty = selectedSpecialtyFilter === 'all' || docSpec === selectedSpecialtyFilter;
  ...
  const handleAddDoctorSubmit = async (e) => {
    ...
    try {
      await addOrgDoctor(name, email, phone, currentUser.id, specialty, password);
      setName('');
      setEmail('');
      setPhone('');
      setPassword('');
      setSpecialty(currentUser.specialty || 'General Practice');
  ...
  const openDoctorDetails = (doc) => {
    ...
    setEditDocSpecialty(doc.specialization || doc.department || 'General Practice');
  };
  ...
  const handleEditDoctorSubmit = (e) => {
    ...
    updateDoctor(selectedDoctor.id, {
      ...
      email: editDocEmail,
      phone: editDocPhone,
      specialization: editDocSpecialty,
  ...
  return (
    <div className={`space-y-stack-lg font-body-md animate-fade-in ${isArabic ? 'text-right' : 'text-left'}`}>
      ...
      <header className="flex justify-between items-center border-b border-border-subtle pb-4">
        ...
        <button
          onClick={() => { setSpecialty(currentUser.specialty || 'General Practice'); setShowAddModal(true); }}
        ...
        <div className="p-4 border-b border-border-subtle flex flex-col sm:flex-row justify-between items-center gap-4 bg-white">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            ...
            <select
              ...
              className={`bg-white border border-border-subtle rounded-lg text-xs py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary shadow-sm text-secondary font-semibold cursor-pointer w-full sm:w-auto ${isArabic ? 'text-right' : 'text-left'}`}
            >
              <option value="all">{isArabic ? 'جميع التخصصات' : 'All Specialties'}</option>
              {SPECIALTIES.map(spec => (
                <option key={spec.val} value={spec.val}>
                  {isArabic ? spec.ar : spec.val}
                </option>
              ))}
            </select>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl border border-border-subtle shadow-lg max-w-md w-full overflow-hidden animate-fade-in">
            ...
            <form onSubmit={handleAddDoctorSubmit} className="p-6 space-y-4 text-start">
              ...
              <div>
                <label className="block text-xs font-semibold text-on-surface-variant mb-1">
                  {isArabic ? 'تخصص الطبيب *' : 'Doctor Specialty *'}
                </label>
                <select
            ...
            {isEditMode ? (
              <form onSubmit={handleEditDoctorSubmit} className="p-6 space-y-4 text-start overflow-y-auto flex-1">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">{isArabic ? 'تخصص الطبيب *' : 'Doctor Specialty *'}</label>
                    <select
                      value={editDocSpecialty} onChange={(e) => setEditDocSpecialty(e.target.value)}
                      className={`w-full px-3 py-2 bg-white border border-border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary text-on-surface ${isArabic ? 'text-right' : 'text-left'}`}
                    >
                      {SPECIALTIES.map(spec => (
                        <option key={spec.val} value={spec.val}>
